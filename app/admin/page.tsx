...
function ProductRow({ product, onSave, onUpload }: { product: Product; onSave: (id: number, values: Record<string, unknown>) => Promise<void>; onUpload: (file: File) => Promise<string> }) {
  ...
  return <tr><td data-label="상품"><div className="table-product"><img src={thumbnailUrl || placeholder} alt="" /><div><strong>{product.name}</strong><small>{product.categorySlug}</small></div></div></td><td data-label="SKU"><code>{product.sku}</code></td><td data-label="상태"><select value={status} onChange={(event) => setStatus(event.target.value)}><option>PUBLISHED</option><option>DRAFT</option><option>ARCHIVED</option></select></td><td data-label="가격 표기"><input value={priceText} onChange={(event) => setPriceText(event.target.value)} /></td><td data-label="공급사 / 썸네일"><span className="supplier-cell">{product.supplierName}</span><input className="image-url-input" value={thumbnailUrl} onChange={(event) => setThumbnailUrl(event.target.value)} placeholder="썸네일 URL" /><label className="upload-button">{uploading ? "업로드 중" : "이미지 업로드"}<input type="file" accept="image/jpeg,image/png,image/webp,image/avif" onChange={handleUpload} disabled={uploading} /></label><GalleryManager productId={product.id} onUpload={onUpload} /></td><td data-label="저장"><button className="save-button" onClick={() => onSave(product.id, { status, priceText, thumbnailUrl })}>저장</button></td></tr>;
}
...
function OrderRow({ order, onSave }: { order: Order; onSave: (id: number, values: Record<string, unknown>) => Promise<void> }) {
  ...
  return <tr><td data-label="주문"><strong>{order.orderNumber}</strong><small className="table-subline">{order.createdAt}</small></td><td data-label="고객"><strong>{order.customerName}</strong><small className="table-subline">{order.customerPhone}</small></td><td data-label="금액">{order.totalAmount.toLocaleString()}원</td><td data-label="결제"><select value={paymentStatus} onChange={(event) => setPaymentStatus(event.target.value)}><option>UNPAID</option><option>PENDING</option><option>PAID</option><option>REFUNDED</option><option>FAILED</option></select></td><td data-label="배송·설치"><select value={deliveryStatus} onChange={(event) => setDeliveryStatus(event.target.value)}><option>NOT_STARTED</option><option>PREPARING</option><option>SHIPPED</option><option>INSTALL_SCHEDULED</option><option>DELIVERED</option><option>CANCELED</option></select></td><td data-label="운송장"><input value={trackingNumber} onChange={(event) => setTrackingNumber(event.target.value)} placeholder="운송장" /></td><td data-label="클레임·환불"><select value={claimStatus} onChange={(event) => setClaimStatus(event.target.value)}><option>NONE</option><option>REQUESTED</option><option>REFUND_REQUESTED</option><option>EXCHANGE_REQUESTED</option><option>COMPLETED</option><option>REJECTED</option></select><input className="number-input claim-refund" type="number" min="0" value={refundAmount} onChange={(event) => setRefundAmount(Number(event.target.value))} placeholder="환불액" /></td><td data-label="저장"><button className="save-button" onClick={() => onSave(order.id, { paymentStatus, deliveryStatus, trackingNumber, claimStatus, refundAmount })}>저장</button></td></tr>;
}
...
function InventoryRow({ product, onSave }: { product: Product; onSave: (id: number, quantity: number, reorderPoint: number) => Promise<void> }) {
  ...
  return <tr className={available <= reorderPoint ? "low-stock-row" : ""}><td data-label="상품"><div className="table-product"><img src={product.thumbnailUrl || placeholder} alt="" /><div><strong>{product.name}</strong><small>{product.sku}</small></div></div></td><td data-label="보유 수량"><input className="number-input" type="number" value={quantity} onChange={(event) => setQuantity(Number(event.target.value))} /></td><td data-label="예약 수량">{product.reserved}</td><td data-label="가용 수량"><strong>{available}</strong></td><td data-label="재주문 기준"><input className="number-input" type="number" value={reorderPoint} onChange={(event) => setReorderPoint(Number(event.target.value))} /></td><td data-label="저장"><button className="save-button" onClick={() => onSave(product.id, quantity, reorderPoint)}>저장</button></td></tr>;
}
